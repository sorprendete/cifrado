    ...
    if (user.eliminado == 1) {
        chatHeader.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px;">
                <button id="btn-back-mobile" class="btn-secondary" style="padding: 5px 10px; font-size: 1rem; border: none; border-radius: 8px;">⬅️</button>
                <div>
                    <h3>Usuario Eliminado</h3>
                    <span class="status-badge" style="background: rgba(255, 68, 68, 0.2); color: #ff4444; border: 1px solid rgba(255, 68, 68, 0.4);">🚫 No puedes responder a esta conversación</span>
                </div>
            </div>
        `;
        ...
        chatHeader.innerHTML = `
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <div>
                    ...
                    <span class="status-badge status-secure">🔒 Conexión E2EE Establecida</span>
                </div>
            </div>
        `;
...
function procesarMensajeDescifrado(msgId, textoDescifrado, timestamp, type) {
    if (textoDescifrado.startsWith('__CTRL_DEL__')) {
        ...
        if (msgDiv) {
            ...
            msgDiv.querySelector('.text').innerHTML = '<i>🚫 Este mensaje fue eliminado</i>';
    ...
    if (type === 'me') {
        actionsHtml = `
            <div class="message-actions">
                ...
                <button class="action-btn btn-delete" title="Borrar">🗑️</button>
            </div>
        `;
    }
    ...
    if (type === 'me' && msgId && !msgId.toString().startsWith('temp_')) {
        ...
        if (btnDel) {
            btnDel.addEventListener('click', () => {
                if (confirm('¿Borrar este mensaje para todos?')) {
...
async function procesarBuzon() {
    ...
    try {
        ...
        if (result.error === 'sesion_invalida') {
            alert('❌ Tu sesión se ha cerrado porque iniciaste sesión en otro dispositivo.');
            localStorage.removeItem('e2ee_identity');
            window.location.href = 'index.html';
            return;
        }
        ...
        if (result.success) {
            ...
            document.querySelectorAll('.btn-revoke-session').forEach(btn => {
                btn.addEventListener('click', async (e) => {
                    ...
                    if (confirm('¿Cerrar la sesión en ese dispositivo?')) {
